...
test_data = data_loading.load_data("test")
...
with tf.Session() as sess:
    ...
    for idx in range(num_batch):
        ...
        refimg = test_data[index, :, :, :].reshape(height, width, 3)
        batchData = test_data[index:index+1]
        start = int(round(time.time() * 1000))
        predMaxOut = sess.run(predmax, feed_dict={x: batchData, keepprob:1.})
        plt.imsave(data_loading.dataset_path+"/test_result/"+str(idx), predMaxOut[0])
        end  = int(round(time.time() * 1000))
